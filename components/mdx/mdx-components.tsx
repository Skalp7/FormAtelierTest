...
function CaseImage({ src, alt }: { src: string; alt: string }) {
  return (
    <figure className="my-14 overflow-hidden rounded-sm">
      <Image src={src} alt={alt} width={1600} height={1000} className="w-full object-cover" />
    </figure>
  );
}
    ...
    <div className="my-12 grid border-y border-line md:grid-cols-3">
      {items.map((item) => (
        <div key={item.label} className="border-line py-8 md:border-r md:px-6 last:md:border-r-0">
          <p className="font-display text-5xl text-ink">{item.value}</p>
...
function Quote({ children }: { children: React.ReactNode }) {
  return (
    <blockquote className="my-16 max-w-4xl border-l border-ink pl-6 font-display text-4xl leading-tight text-ink md:text-6xl">
      {children}
    </blockquote>
  );
}
...
function BeforeAfter({
  ...
  return (
    <div className="my-14 grid gap-4 md:grid-cols-2">
      ...
      <figure>
        <div className="relative aspect-[4/3] overflow-hidden rounded-sm">
          <Image src={after} alt={afterAlt} fill className="object-cover" sizes="(min-width: 768px) 50vw, 100vw" />
        </div>
...
function VideoEmbed({ title, src }: { title: string; src: string }) {
  return (
    <div className="my-14 aspect-video overflow-hidden rounded-sm bg-ink">
      <iframe
        title={title}
        src={src}
